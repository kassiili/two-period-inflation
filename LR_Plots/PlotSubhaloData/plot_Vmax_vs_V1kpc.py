...
class plot_slice:
    ...
    def divideIntoSubgroups(self):
        ...
        SGNfromPartData = read_dataset(self.part_type, 'SubGroupNumber')

        coordsBySubGroup = dict()
        for subGroupNumber in pd.unique(SGNfromPartData):
            coordsBySubGroup[subGroupNumber] = self.coords[SGNfromPartData == subGroupNumber,:] 
            
        return coordsBySubGroup


    def calcVelocitiesAt1kpc(self):
        cops = read_subhaloData('CentreOfPotential')
        ...
        for idx, subGroupNumber in enumerate(self.subGroupNumbers):
            cop = cops[self.subGroupNumbers == subGroupNumber,:][0,:]    # Find center of potential (np.where(...) returns an array of indeces, which fulfill the condition – we only need the first element, since they all correspond to the same subhalo)
    ...
    def __init__(self, part_type):
        ...
        maskSat = np.logical_and(self.maxVelocities > 0, self.subGroupNumbers != 0)
        maskIsol = np.logical_and(self.maxVelocities > 0, self.subGroupNumbers == 0)
