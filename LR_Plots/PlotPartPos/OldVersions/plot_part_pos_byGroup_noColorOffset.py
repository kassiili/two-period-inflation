...
class plot_slice:
    ...
    def plot(self):
        ...
        start = time.clock()
        #Set the color of the largest subgroup so that the other subgroups are clearly distinguishable from it. To accoplish this, the colors of the other subgroups are offset by 30.
        centralSubGroupCoords = self.coordsBySubGroup.pop(0)
        col = [1] * centralSubGroupCoords[:,0].size
        x = centralSubGroupCoords[:,1].tolist()
        y = centralSubGroupCoords[:,2].tolist()
        i = 10
