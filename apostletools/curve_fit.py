...
def median_trend_fixed_bin_width(x, y, n_bins=5):
    """ Compute medians in bins that split the range of x equally. """
    ...
    xmin = np.min(x)
    xmax = np.max(x) + 10**-5
    bin_lim = [xmin + i * (xmax - xmin) / n_bins
               for i in range(n_bins + 1)]
    bin_masks = [np.logical_and(x >= bin_lim[i], x < bin_lim[i+1])
                 for i in range(n_bins)]

    med_x = np.array([np.median(x[mask]) for mask in bin_masks])
    med_y = np.array([np.median(y[mask]) for mask in bin_masks])

    return med_x, med_y

def bin_array(arr, n_bins):

    # Get the bin sizes, s.t. they differ by at most 1, and the last bin is
    # largest, in the array ´n_bin´:
    n = arr.size
    n_last_bin = np.ceil(n / n_bins)
    n_bin = np.array([n_last_bin for i in range(n_bins)])
    n_bin[1:1 + (n_bins - n % n_bins)] -= 1
    n_bin = n_bin[::-1]

    bar_lim = np.append([0], np.cumsum(n_bin)).astype(int)

    return [arr[bar_lim[i]:bar_lim[i + 1]] for i in range(n_bins)]

def median_trend_fixed_bin_points(x, y, n_bins=5):
    """ Compute medians in bins that have an equal number of points.

    Notes
    -----
    This is FLAWED: median bins for x and y do not have the same points...
    """
    ...
    med_x = np.array([np.median(xbin) for xbin in bin_array(x, n_bins)])
    med_y = np.array([np.median(ybin) for ybin in bin_array(y, n_bins)])
...
#
# def running_median(x, y, f_kernel=0.2, n_bar=10):
#
#     # Sort by x:
#     argsort = np.argsort(-x)
#     x = x[argsort]
...
#
#     maxx = x[0]; minx = x[-1]
#     dx_tot = abs(maxx-minx)
#     dx_jump = dx_tot / n_bar
#     dx_kernel = f_kernel * dx_tot
#     dx = np.array([[minx + i * dx_jump,
#                     minx + i * dx_jump + dx_kernel]
#                    for i in range(n_bar)])
#
#     med_x = np.array([np.median(x[i:(i+n_kernel)])
#                       for i in range(0, x.size-n_kernel, n_jump)])
#     med_y = np.array([np.median(y[i:(i+n_kernel)])
#                       for i in range(0, x.size-n_kernel, n_jump)])
#
#     return med_x, med_y
#
# def running_median(x, y, n_kernel=10, n_jump=5):
#
#     # Sort by x:
#     argsort = np.argsort(x)
#     x = x[argsort]
...
#     return med_x, med_y
#
# def running_median_dum(x, y, n_kernel=None, min_dx=None):
#
#     if n_kernel is None:
#         n_kernel = 5
#     else:
